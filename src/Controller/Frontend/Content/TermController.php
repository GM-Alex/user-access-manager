...
class TermController extends ContentController
{
    ...
    private function getAllPostsForTerm(string $termType, int|string|null $termId): array
    {
        $termTreeMap = $this->objectMapHandler->getTermTreeMap();
        $childTerms = $termTreeMap[ObjectMapHandler::TREE_MAP_CHILDREN][$termType][$termId] ?? [];
        $fullTerms = [$termId => $termType] + $childTerms;

        $posts = [];
        $termPostMap = $this->objectMapHandler->getTermPostMap();

        foreach (array_keys($fullTerms) as $fullTermId) {
    ...
    private function getVisibleElementsCount(string $termType, int|string|null $termId): int
    {
        ...
        if (isset($this->visibleElementsCount[$key]) === true) {
        ...
        foreach ($this->getAllPostsForTerm($termType, $termId) as $postId => $postType) {
            $isPostTypeHidden = $postTypeHiddenMap[$postType] ?? true;

            if ($isPostTypeHidden === false
     ...
     * @throws UserGroupTypeException
     */
    private function processTermMenuItem(object $item): bool
    {
        $term = $this->processTerm($this->objectHandler->getTerm($item->object_id), $isEmpty);
    ...
    public function getTermArguments(array $arguments): array
    {
        ...
        $arguments['exclude'] = array_unique(array_merge($exclude, $this->accessHandler->getExcludedTerms()));
