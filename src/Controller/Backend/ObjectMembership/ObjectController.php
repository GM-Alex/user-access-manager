...
class ObjectController extends Controller
{
    ...
    protected function setObjectInformation(
        ...
        $this->objectInformation->setObjectType($objectType)
            ...
            ->setObjectUserGroups($objectUserGroups ?? [])
    ...
    {
        return $this->groupsFromName ?? self::DEFAULT_GROUPS_FORM_NAME;
    }
    ...
    public function sortUserGroups(array &$userGroups): void
    {
        ...
        uasort(
            ...
            ) use ($notLoggedInUserGroupId) {
                if ($userGroupOne->getId() === $notLoggedInUserGroupId) {
                    return 1;
                }

                if ($userGroupTwo->getId() === $notLoggedInUserGroupId) {
                    return -1;
                }
    ...
    {
        $recursiveMembership = [];
        $recursiveMembershipForObject = $userGroup->getRecursiveMembershipForObject(
            $this->objectInformation->getObjectType(),
            $this->objectInformation->getObjectId()
        );
        ...
        foreach ($recursiveMembershipForObject as $assignmentInformation) {
            foreach ($assignmentInformation as $membershipObjectId => $information) {
                try {
                    ...
                    $recursiveMembership[$typeName][$membershipObjectId] = $membershipHandler->getObjectName(
                        $membershipObjectId,
                        $typeName
                    );
    ...
    public function checkRightsToEditContent(): void
    {
        ...
        if ($postIdParameter !== null) {
            foreach ((array) $postIdParameter as $postId) {
    ...
    public function saveObjectData(
        ...
        if (($isUpdateForm === true && $hasRights === true) === false && $force === false) {
            return;
        }
