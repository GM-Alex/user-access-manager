...
class UserGroupController extends Controller
{
    ...
    public function insertUpdateUserGroupAction(): void
    {
        ...
        $userGroup->setDescription($this->getRequestParameter('userGroupDescription'));
        $userGroup->setReadAccess($this->getRequestParameter('readAccess'));
        $userGroup->setWriteAccess($this->getRequestParameter('writeAccess'));
        $userGroup->setIpRange($this->getRequestParameter('ipRange'));

        if ($userGroup->save() === false) {
            return;
        }

        $this->assignRolesFromRequest($userGroup);

        $this->setUpdateMessage(
            $userGroupId === null ? TXT_UAM_GROUP_ADDED : TXT_UAM_USER_GROUP_EDIT_SUCCESS
        );

        $this->userGroupHandler->addUserGroup($userGroup);
    }

    private function assignRolesFromRequest(UserGroup $userGroup): void
    {
    ...
    private function isDefaultTypeAdd(
        ...
        $userGroupInfo = $defaultUserGroups[$userGroupId] ?? [];

        if (isset($userGroupInfo['id']) === false || (string) $userGroupInfo['id'] !== $userGroupId) {
            return false;
        }

        $fromTime = empty($userGroupInfo['fromTime']) ? null : $userGroupInfo['fromTime'];
        $toTime = empty($userGroupInfo['toTime']) ? null : $userGroupInfo['toTime'];
