...
Plugin URI: http://www.gm-alex.de/
Version: 0.6
Author: <a href="http://www.gm-alex.de/">Alexander Schneider</a>
...
if (!class_exists("UserAccessManager"))
{
	class UserAccessManager
	{
										...
										<tr class="alternate" id="group-<?php echo $accessgroup['ID']; ?>">
											...
											<td>
												...
												if($posts)
												{
													foreach($posts as $post)
													{
														$cur_post = get_post($post['post_id']);
		...
		{
			$cur_post = get_post($dummy_id = null);
		...
		function redirect_user()
		{
			...
			{
				$cur_post = get_post($_GET['getfile']);
		...
		function get_file($URL, $attachment_ID = null)
		{
			$cur_post = get_post($ID = null);
			$cur_parent = get_post($cur_post->post_parent);
...
{	
	...
	add_filter('wp_get_attachment_url', array(&$userAccessManager, 'get_file'), 1);
